...
public class TransferToAccountCommand implements CustomCommand {
    ...
    public Batch execute(Batch requestBatch) {
        ...
        String toAccountId = requestBatch.getBatchMap().get(RequestParameter.TO_ACCOUNT_ID).toString();
        try {
            result = accountService.makeTransferToAccount(accountId, toAccountId, moneyAmount);
