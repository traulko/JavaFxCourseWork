...
public class UnblockAccountCommand implements CustomCommand {
    ...
    public Batch execute(Batch requestBatch) {
        ...
        String token = requestBatch.getBatchMap().get(RequestParameter.ACCOUNT_TOKEN).toString();
        Integer accountId = (int) requestBatch.getBatchMap().get(RequestParameter.ACCOUNT_ID);
        try {
            result = accountService.unblockAccount(accountId, token);
        } catch (ServiceException e) {
            LOGGER.log(Level.ERROR, "Error while unblocking account", e);
        }
